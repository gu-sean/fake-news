...
_NB_OUT_DIR = os.path.join(PROJECT_DIR, "models", "naive_bayes")
os.makedirs(_NB_OUT_DIR, exist_ok=True)

# 학습 중 화면과 파일에 남길 로그 파일 경로입니다.
LOG_PATH = os.path.join(_NB_OUT_DIR, "naive_V1_log.txt")

# 현재까지 가장 좋은 모델을 저장할 pkl 파일 경로입니다.
BEST_MODEL_PATH = os.path.join(_NB_OUT_DIR, "best_model_naive_V1.pkl")

# 현재까지 가장 좋은 설정과 점수를 사람이 읽는 txt로 저장할 경로입니다.
BEST_CONFIG_PATH = os.path.join(_NB_OUT_DIR, "best_config_naive_V1.txt")
