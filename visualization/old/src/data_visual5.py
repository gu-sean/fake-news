...
class TokenDocumentStream:
    """Stream real-news documents as pre-tokenized strings for TF-IDF vectorization."""
    ...
    def __iter__(self):
        ...
        for chunk in pd.read_csv(
            ...
            real_messages = chunk.loc[chunk[LABEL_COL] == self.real_label, TEXT_COL]

            for message in real_messages.dropna().astype(str):
...
def calculate_tfidf_scores(
    ...
    """Calculate average TF-IDF score per word across real-news documents."""
...
def plot_tfidf_scores(
    ...
    draw_wordcloud(
        ...
        title=f"Real News Korean Top {top_cloud_words} TF-IDF Word Cloud",
        font_path=font_path,
    ...
    draw_barplot(
        ax=axes[0][1],
        scores=korean_scores,
        title=f"Real News Korean Top {top_bar_words} TF-IDF Terms",
    ...
    draw_wordcloud(
        ...
        title=f"Real News English Top {top_cloud_words} TF-IDF Word Cloud",
        font_path=None,
        top_words=top_cloud_words,
        colormap="Dark2",
    )
    draw_barplot(
        ax=axes[1][1],
        scores=english_scores,
        title=f"Real News English Top {top_bar_words} TF-IDF Terms",
        top_words=top_bar_words,
        palette="Set3",
    )

    fig.suptitle("Real News Important Terms by TF-IDF", fontsize=18, y=0.995)
    fig.tight_layout()
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Visualize important real-news terms with TF-IDF."
    )
    parser.add_argument("--csv", default="unified_news_refined.csv")
    parser.add_argument("--output", default="data_visual5_real_tfidf.png")
    parser.add_argument("--real-label", type=int, default=0)
...
def main() -> None:
    """Run the full real-news TF-IDF visualization pipeline."""
    args = parse_args()
