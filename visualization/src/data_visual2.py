...
# 명사, 동사, 형용사에 사용되는 Kiwi Korean POS tags와 NLTK English POS prefixes.
...
# 지나치게 자주 출현하는 경향이 있지만 분석에 거의 의미를 부여하지 않는 일반적인 단어들.
...
def normalize_korean_token(token: str, tag: str) -> str:
    """Attach '다' to Korean verb/adjective stems so they are easier to read."""
    if tag in {"VV", "VA"}:
        return f"{token}다"
    return token
...
def count_pos_words(
    ...
    """Count Korean and English words from fake-news rows using POS and length filters."""
    kiwi = Kiwi()
    ...
    for chunk in pd.read_csv(
        ...
        # Only fake-news rows are used because this visualization focuses on fake-news vocabulary.
        # 이 시각화는 가짜 뉴스 어휘에 중점을 두기 때문에 가짜 뉴스 행만 사용됩니다.
        fake_messages = chunk.loc[chunk[LABEL_COL] == fake_label, TEXT_COL].dropna().astype(str)
        ...
        for message in fake_messages:
            ...
            # 한국어 단어는 Kiwi로 분석되므로 의미 있는 POS 카테고리만 남습니다.
            ...
            # 영어 단어를 먼저 수집한 다음 속도를 위해 일괄적으로 POS 태그를 붙입니다.
...
def plot_word_frequency(
    ...
    draw_wordcloud(
        ...
        title=f"Korean Top {top_cloud_words} Word Cloud",
        font_path=font_path,
    ...
    draw_barplot(
        ax=axes[0][1],
        counts=korean_counts,
        title=f"Korean Top {top_bar_words} Word Frequency",
    ...
    draw_wordcloud(
        ...
        title=f"English Top {top_cloud_words} Word Cloud",
        font_path=None,
        top_words=top_cloud_words,
        colormap="Dark2",
    )
    draw_barplot(
        ax=axes[1][1],
        counts=english_counts,
        title=f"English Top {top_bar_words} Word Frequency",
        top_words=top_bar_words,
        palette="Set3",
    )

    fig.suptitle("Fake News Word Frequency by Language", fontsize=18, y=0.995)
    fig.tight_layout()
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Visualize noun, verb, and adjective frequencies from fake-news clean_message text."
    )
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--output",
        default="data_visual2_word_frequency.png",
...
def main() -> None:
    """Run the full fake-news word-frequency visualization pipeline."""
    args = parse_args()
